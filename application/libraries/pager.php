...
class Pager
{
	...
	public function __construct($data = array())
	{
		...
		$this->page_name  		= !empty($data['page_name']) ? $data['page_name'] : 'curr_page';
    ...
    public function up_page($name = '上一页')
    {
    	if($this->now_page != 1)
    	{
    		return "<div class=\"nepr_btn\">".$this->_get_link($this->now_page - 1, $name)."</div>";
    	}else{
            return "<div class=\"nepr_btn\">".$this->_get_link($this->now_page, $name)."</div>";
        }
    }
    ...
    protected function show_1()
    {
    	...
    	for ($i = $begin; $i <= $begin + $plus * 2;$i++)
    	{
    		...
    		if($i == $this->now_page)
    		{
    			$return .= "<a style='color:red;' class='now_page'>$i</a>\n";
    		}
    		else
    		{
    			$return .= $this->_get_link($i, $i) . "\n";
