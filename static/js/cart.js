...
(function () {
  ...
  $('.buy-btn').click(function (e) {
    ...
    if (shopping_cart.length <= 0 && $('#picmenu>.ui-draggable').length==0) {
      ...
      dom += '</table><div class="foot"><span class="total">共：' + get_total() + ' 元</span>';
      if(open_account){
        dom += '<button type="button" id="buy-go" class="btn">提交订单</button>';
      }else{
        dom += '<button type="button" class="btn">余额不足</button>';
      }
      ...
      $('#buy-go').unbind('click').bind('click', function () {
        ...
        $.ajax({
          ...
          data: "list=" + encodeURIComponent(JSON.stringify(shopping_cart)) +"&shop_name=" + shop_name + "&shop_id=" + shop_id,
          dataType: 'json',
  ...
  {
    return true;
    var user_money = $("#hidden_money").val();
    var total_account = $("#hidden_total").val();
    if(parseInt(user_money) >= 0 && parseInt(user_money) < parseInt(total_account))
    {
      $("#notice").html("<h5>你的账户余额不足!</h5>");
      return false;
    }else{
      $("#notice").html("");
      return true;
